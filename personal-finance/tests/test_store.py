...
def test_migration_rolls_back_on_failure(
    ...
    - The bad statement is terminated with ``;`` so the splitter emits
      ...
      back too - which is the PR#9/10 review finding the umbrella's
      atomic-migration story is supposed to fix).
    - After the failure, we assert both ``sqlite_master`` is clean AND
      ``schema_version(conn) == 0``. The latter would also catch a
      regression where the INSERT lands in autocommit mode.
    """
    monkeypatch.setattr(
        store,
        "_load_migration_sql",
    ...
    try:
        ...
        # And the version-tracker row in particular: 0 means "no
        # migration applied", as required.
